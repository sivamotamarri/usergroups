class Receipt < ActiveRecord::Base
  ...
  belongs_to :my_message, :foreign_key => "notification_id"
  ...
  scope :conversation, lambda { |conversation|
    joins(:my_message).where('notifications.conversation_id' => conversation.id)
  }
  ...
  def conversation
    return my_message.conversation if my_message.is_a? MyMessage
    return nil
  end
  ...
  if Mailboxer.search_enabled
    searchable do
      text :subject, :boost => 5 do
        my_message.subject if my_message
      end
      text :body do
        my_message.body if my_message
      end
      integer :receiver_id
    end
  end
