class Conversation < ActiveRecord::Base
  attr_accessible :subject
	...
	has_many :my_messages, :dependent => :destroy
	has_many :receipts, :through => :my_messages
  ...
  scope :participant, lambda {|participant|
    ...
    where('notifications.type'=> MyMessage.name).
  ...
  #Returns an array of participants
	...
	def original_message
		@original_message = self.my_messages.find(:first, :order => 'created_at') if @original_message.nil?
	...
	def last_message
		@last_message = self.my_messages.find(:first, :order => 'created_at DESC') if @last_message.nil?
		return @last_message
	end
  ...
  #Returns the number of messages of the conversation
	def count_messages
		return MyMessage.conversation(self).count
	end
